...
Webholm uses a layered test pyramid so most regressions are caught before Electron packaging or Playwright runs.
...
| **Contract** | `shared/src/contract/`, `src/options/webholmJsonContract.test.ts` | One ruleset for `webholm.json`: builder output (`mapAppOptionsToOutputOptions`) must satisfy `validateWebholmJsonContract` and runtime parsing (`parseRuntimeConfigJson`). |
...
- **webholm.json:** `shared/src/contract/webholmJsonContract.ts` defines required runtime fields; `webholmJsonContract.test.ts` (shared + src) ties builder output to that contract and to `parseRuntimeConfigJson`.
...
Integration and Playwright tests are the final safety net for end-to-end behavior, not the first place to catch a wrong default or a missing `webholm.json` field. Prefer contract and unit tests for those.
